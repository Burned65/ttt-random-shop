---- Traitor equipment menu, randomized so that not every item gets shown every round
---- current Author: Alyx
---- Version 1603_22
---- 
...
local function TraitorMenuPopup()
   ...
   if (rs_number_cvar:GetInt() == 0) then   --if CVar set to 0 use all items in shop
      items = GetEquipmentForRole(ply:GetRole())
   elseif(rs_number_cvar:GetInt() == 1) then --dirty workaround to enable only 1 item in shop
     Yolo = tmpItems[math.random(table.maxn(tmpItems))]   
     table.insert(items,1,Yolo)
   elseif  not next(randomShopItems) then

      if (rs_armour_cvar:GetInt() == 1) then
         Yolo = tmpItems[1]   
         table.insert(items,1,Yolo)
      end
      if (rs_radar_cvar:GetInt() == 1) then
         Yolo = tmpItems[2]   
         table.insert(items,1,Yolo)
      end

...
local function TraitorMenuPopup()
   ...
   --- fills the shop with items -> edited code for randomshop
   ...
   local Yolo
   local items = {}
   local randomshop_cvar = GetConVar( "sv_randomshop_items") --Serverside Convar to determin the number of items in shop
   local itemnumber_int = (randomshop_cvar:GetInt()-1)       --Subtract 1 because we start counting from 0


   math.randomseed(os.time())
   math.random(); math.random()

   if (randomshop_cvar:GetInt() == 0) then   --if CVar set to 0 use all items in shop
      items = GetEquipmentForRole(ply:GetRole())
   elseif(randomshop_cvar:GetInt() == 1) then --dirty workaround to enable only 1 item in shop
     Yolo = tmpItems[math.random(table.maxn(tmpItems))]   
     table.insert(items,1,Yolo)
   elseif  not next(randomShopItems) then
         ...
         until (table.Count(items) > itemnumber_int )
